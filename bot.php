...
function progress($resource, $download_size, $downloaded, $upload_size, $uploaded)
{
    ...
    if ($upload_size > 100) {
        if ($uploaded % 100 == 0) {
            ...
            $curu = $tmp;
            $res = $Tl->makeHTTPRequest('editMessageText', ['inline_message_id' => $messageId, 'text' => buidProgress($uploaded / 1024, $upload_size / 1024, true), 'parse_mode' => 'Html', 'reply_markup'=> json_encode(
        ...
        $curd = $tmp;
        $res = $Tl->makeHTTPRequest('editMessageText', ['inline_message_id' => $messageId, 'text' => buidProgress($downloaded / 1024, $download_size / 1024), 'parse_mode' => 'Html', 'reply_markup'=> json_encode(
        InlineKeyBoardMarkUp::build(false, [[
            InlineKeyBoardItem::build('کنسل', null, "del$fromUsetID"),
        ]]))]);
...
if (isset($_GET['q'])) {
    ...
    $messageId = $_GET['m'];
    $fromUsetID = $_GET['f'];
    $res = $Tl->sendMediaByContent('Document', downloadFile($query), ['chat_id'=>'@'."$FilesCh"], basename($query), true);
    if ($res['ok']) {
        $fileUrl = "https://t.me/$FilesCh/".$res['result']['message_id'];
        $res = $Tl->makeHTTPRequest('editMessageText', ['inline_message_id'=>$messageId, 'text'=>"File : \n<a href='$fileUrl'>&#160;</a>", 'parse_mode'=>'Html']);

       // $Tl->sendToMe(json_encode($res));
    } else {
        $res = $Tl->makeHTTPRequest('editMessageText', ['inline_message_id'=>$messageId, 'text'=>'نشد :|', 'parse_mode'=>'Html']);
    }
}
